...
export const ProductCard = ({ product }: ProductCardProps) => {
  ...
  const cartItem = cart?.products.find((item) => item.productId._id === product.id);
  ...
  return (
    <Link href={`/product/${product.id}`} passHref>
      <Card className="group overflow-hidden bg-gradient-to-br from-card via-card to-card/95 border border-border/50 transition-all duration-300 hover:shadow-lg hover:border-primary/30 hover:-translate-y-0.5 rounded-xl sm:rounded-2xl cursor-pointer">
